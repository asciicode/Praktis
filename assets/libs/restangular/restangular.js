          ...
          function createServiceForConfiguration(config) {
              ...
              function resolvePromise(deferred, response, data) {
                ...
                if (config.fullResponse) {
                  ...
                  /* FIXME: Jimmy on 20140717
                   * Since we have some existing code in data, we cannot modify all server side service for DealStation Wave 1 delivery, 
                   * We hack to include the request data in the response, so that we can avoid out of order response arrival 
                   * */
                  if (data) data.requestParams = response.config.params;
